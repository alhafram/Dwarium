...
async function equipRequest(baseUrl, id) {
    var rnd_url = '&_=' + (new Date().getTime() + Math.random())
    let req =
        `fetch('${baseUrl}/action_run.php?code=PUT_ON&url_success=user_iframe.php%3Fgroup%3D2%26update_swf%3D1&url_error=user_iframe.php%3Fgroup%3D2%26update_swf%3D1&artifact_id=${id}&in[slot_num]=0&in[variant_effect]=0&ajax=1${rnd_url}', {
          'headers': {
            'accept': '*/*',
            'accept-language': 'en-GB,en-US;q=0.9,en;q=0.8,ru;q=0.7',
            'cache-control': 'no-cache',
...
async function changeStyle(baseUrl, zikkuratId, styleId) {
    let req = `fetch('${baseUrl}/action_run.php', {
      'headers': {
        'content-type': 'application/x-www-form-urlencoded',
        'upgrade-insecure-requests': '1'
      },
      'referrer': '${baseUrl}/action_form.php?${Math.random()}&artifact_id=${zikkuratId}&in[param_success][url_close]=user.php%3Fmode%3Dpersonage%26group%3D2%26update_swf%3D1',
      'referrerPolicy': 'no-referrer-when-downgrade',
...
// async function getMagicSchools(baseUrl, zikkuratId) {
//     let req = `fetch(
//       '${baseUrl}/action_form.php?${Math.random()}&artifact_id=${zikkuratId}&in[param_success][url_close]=user.php%3Fmode%3Dpersonage%26group%3D2%26update_swf%3D1', {
//           'headers': {
...
//               'upgrade-insecure-requests': '1'
//           },
//           'referrer': '${baseUrl}/user_iframe.php?group=2',
//           'referrerPolicy': 'no-referrer-when-downgrade',
//           'body': null,
//           'method': 'GET',
//           'mode': 'cors',
