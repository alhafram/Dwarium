...
function putOffItem(element, remove, fake) {
    if(element.childElementCount > 0) {
        ...
        if(!fake) {
            ...
            fetch(`http://w1.dwar.ru/action_run.php?code=PUT_OFF&url_success=user_iframe.php%3Fgroup%3D1%26update_swf%3D1&url_error=user_iframe.php%3Fgroup%3D1%26update_swf%3D1&artifact_id=${item.attributes.itemId.value}&ajax=1${rnd_url}`, {
                  "headers": {
                    "accept": "*/*",
...
function putOnItem(item, fake) {
    ...
    if(!item.attributes.copy && !fake) {
        var rnd_url = '&_=' + (new Date().getTime() + Math.random());
        fetch(`http://w1.dwar.ru/action_run.php?code=PUT_ON&url_success=user_iframe.php%3Fgroup%3D2%26update_swf%3D1&url_error=user_iframe.php%3Fgroup%3D2%26update_swf%3D1&artifact_id=${item.attributes.itemId.value}&in[slot_num]=0&in[variant_effect]=0&ajax=1${rnd_url}`, {
          "headers": {
            "accept": "*/*",
            "accept-language": "en-GB,en-US;q=0.9,en;q=0.8,ru;q=0.7",
            "cache-control": "no-cache",
