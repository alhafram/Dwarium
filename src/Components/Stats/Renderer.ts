...
async function renderFights(initialState: StatsWindowState) {
    for(let i = 0; i < initialState.selectedDayFightIds.length; i++) {
        ...
        const isDeletedFight =
            doc.querySelector('body > table > tbody > tr:nth-child(2) > td > div > div.bg-l > div > div > div > div > div > div > div > p')?.textContent ==
            '\n\t\t\t\t\t\t\t\t\t\t\t\t\t\t\tБой не найден!\t\t\t\t\t\t\t\t\t\t\t\t\t\t'
        ...
        const strignElement = isDeletedFight
            ? `<p class="text-secondaryLightDark dark:text-secondaryLight"> <a id='${id}' href='#'>Бой удален</a></p>`
            : `<p class="text-secondaryLightDark dark:text-secondaryLight">${formattedDate} <a id='${id}' href='#'>${title}</a> ${duration}</p>`
        const parser = new DOMParser()
